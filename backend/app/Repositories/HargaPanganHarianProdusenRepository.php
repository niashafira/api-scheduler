...
class HargaPanganHarianProdusenRepository
{
    /**
    ...
    public function storeData(array $data): void
    {
        try {
            foreach ($data as $record) {
                HargaPanganHarianProdusen::updateOrCreate(
                    [
    ...
    {
        $query = HargaPanganHarianProdusen::whereBetween('tanggal', [$startDate, $endDate]);
    ...
    {
        return HargaPanganHarianProdusen::where('kode_wilayah', $kodeWilayah)
    ...
    public function clearOldData(int $daysOld = 30): int
    {
        ...
        return HargaPanganHarianProdusen::where('tanggal', '<', $cutoffDate)->delete();
    }
