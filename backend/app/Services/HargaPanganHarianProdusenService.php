...
use App\Models\HargaPanganHarianProdusen;
use App\Repositories\HargaPanganHarianProdusenRepository;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Storage;
use Carbon\Carbon;

class HargaPanganHarianProdusenService
{
     ...
     */
    public function getHargaPanganHarianProdusenData(string $startDate, string $endDate, ?string $kodeWilayah = null): int
    {
        try {
            ...
            $this->logger->error("Error in getHargaPanganHarianProdusenData: " . $e->getMessage());
            throw $e;
        }
    }
    ...
    protected function fetchDataForRegion(string $kodeWilayah, string $startDate, string $endDate): array
    {
        try {
            ...
            foreach ($transformedData as $record) {
                $this->hargaPanganHarianProdusenRepository->storeData([$record]);
            }
    ...
    {
        try {
            // Build URL with path parameters: /{startDate}/{endDate}/1/{kodeWilayah}
            $apiUrl = $this->externalApiUrl . '/' . $startDate . '/' . $endDate . '/1/' . $kodeWilayah;
